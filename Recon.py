...
REPORTS_FILE = "reconciliation_reports.csv" # Define the filename for your reports CSV
...
import pandas as pd # Ensure pandas is imported if this is a standalone file
...
def load_all_reports():
    """Loads all stored reconciliation reports from a CSV file."""
    try:
        # Ensure the 'Month & Year' column is read as string/object to prevent date parsing issues
        # and handle potential mixed types or missing file gracefully.
        df = pd.read_csv(REPORTS_FILE, dtype={'Month & Year': str})
        return df
    except FileNotFoundError:
        # Return an empty DataFrame with the expected columns if the file doesn't exist
        return pd.DataFrame(columns=[
            "Provider name", "Currency", "Month & Year", "Var brought forward (m-1)",
            "# of Transactions", "Partner Statement", "Treasury Records", "Variance",
            "% accuracy", "Status", "Comments", "Access feedback"
        ])
    except Exception as e:
        st.error(f"Error loading past reports: {e}. Starting with an empty report list.")
        return pd.DataFrame(columns=[
            "Provider name", "Currency", "Month & Year", "Var brought forward (m-1)",
            "# of Transactions", "Partner Statement", "Treasury Records", "Variance",
            "% accuracy", "Status", "Comments", "Access feedback"
...
def save_all_reports(df):
    """Saves the current DataFrame of all reconciliation reports to a CSV file."""
    try:
...
def generate_excel_summary_row(summary_dict, provider_name, selected_country, recon_month_year):
    """
    Generates a dictionary representing a single row of the Excel-like reconciliation summary.
    Now retrieves currency directly from summary_dict, and uses updated key names.
    """
    currency = summary_dict.get("Currency", "N/A") # Get currency from the summary_dict

    # --- MODIFIED: Use the exact keys from the reconciliation functions' summary output ---
    total_internal_records_for_accuracy = summary_dict.get("Total Internal Records (for recon)", 0)
    total_matched_transactions = summary_dict.get("Total Matched Transactions (All Stages)", 0) # Updated key

    # Calculate percentage accuracy, handling division by zero
    percentage_accuracy = (total_matched_transactions / max(1, total_internal_records_for_accuracy)) * 100

    # Determine status based on final discrepancy amount
    status = "Matched" if abs(summary_dict.get("Overall Discrepancy (Original)", 0)) < 0.01 else "Unmatched" # Updated key

    excel_summary_data = {
        "Provider name": provider_name,
        "Currency": currency,
        "Month & Year": recon_month_year,
        #"Var brought forward (m-1)": 0.00, # Placeholder, as this requires previous month's data
        # Sum of total matched + remaining unmatched
        "# of Transactions": total_matched_transactions + \
                             summary_dict.get("Unmatched Internal Records (Final)", 0) + \
                             summary_dict.get("Unmatched Bank Records (Final)", 0), # Updated keys
        "Partner Statement": summary_dict.get("Total Bank Credits (Original)", 0.00), # Updated key
        "Treasury Records": summary_dict.get("Total Internal Credits (Original)", 0.00), # Updated key
        "Variance": summary_dict.get("Overall Discrepancy (Original)", 0.00), # Updated key
        "% accuracy": f"{percentage_accuracy:.2f}%",
        "Status": status,
        "Comments": "-", # Placeholder
        "Access feedback": "-" # Placeholder
    }
    return excel_summary_data
        ...
        if internal_file is None or bank_file is None:
            ...
            if recon_func:
                ...
                if summary: # A simple check to ensure summary was populated
                    ...
                    # ---  Add Summary to Reports Button ---
                    if st.button("Add Current Summary to Reports"):
                        # Append the current summary row to the session state DataFrame
                        # Create a temporary DataFrame for the new row to use pd.concat
                        new_row_df = pd.DataFrame([current_excel_summary])
                        st.session_state.all_reconciliation_reports = pd.concat([st.session_state.all_reconciliation_reports, new_row_df], ignore_index=True)
                        save_all_reports(st.session_state.all_reconciliation_reports)
                        st.success("Summary added to reports page!")
                    # --- Add Summary to Reports Button ---
...
def reports_page():
    """
    Displays all saved reconciliation reports and provides options to download or clear them.
    """
    st.header("Reconciliation Reports History")
    st.write("Here you can view all the reconciliation summaries that have been saved.")

    if 'all_reconciliation_reports' in st.session_state and not st.session_state.all_reconciliation_reports.empty:
        st.subheader("Saved Reconciliation Summaries")

        # Display the reports DataFrame
        st.dataframe(st.session_state.all_reconciliation_reports.style.format({
            'Var brought forward (m-1)': '{:,.2f}',
            'Partner Statement': '{:,.2f}',
            'Treasury Records': '{:,.2f}',
            'Variance': '{:,.2f}'
        ...
        with col_download:
            ...
            csv_data = st.session_state.all_reconciliation_reports.to_csv(index=False).encode('utf-8')
            st.download_button(
        ...
        with col_clear:
            ...
            if st.button("Clear All Reports", use_container_width=True, help="This will permanently delete all saved reports."):
                if st.session_state.all_reconciliation_reports.empty:
                    st.info("No reports to clear.")
                else:
                    # Confirmation step before clearing
                    st.warning("Are you sure you want to permanently delete all reconciliation reports? This action cannot be undone.")
                    if st.button("Confirm Clear All Reports", key="confirm_clear_reports"):
                        st.session_state.all_reconciliation_reports = pd.DataFrame() # Reset to empty DataFrame
                        save_all_reports(st.session_state.all_reconciliation_reports) # Save the empty DataFrame
                        st.success("All reports have been cleared.")
                        st.rerun() # Rerun to update the display
    else:
        st.info("No reconciliation reports saved yet. Run a reconciliation on the 'Reconciliation' page and add its summary to save reports.")

    st.markdown("---") # Visual separator
    if st.button("Back to Home", key="back_to_home_reports_page"):
        st.session_state.page = "home"
        st.rerun()
