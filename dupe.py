...
MONTH_FILTER_BANKS = ["Verto"] # Needing month filter
...
def reconcile_flutterwave_ngn(internal_file_obj, bank_file_obj):
    ...
    try:
        ...
        # --- 7. Same-Day Aggregation Matching ---
        ...
        accuracy = (total_matched / len(FW_ng_hex_df_recon) * 100) if len(FW_ng_hex_df_recon) > 0 else 0
        ...
        summary = {
            ...
            "% accuracy": f"{accuracy:.2f}%",
            "Status": "Matched" if abs(discrepancy_amount) < 0.01 else "Partial",
            "Timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "Comments": "",
            "Matching Breakdown": {
                "Exact Matches": len(matched_transactions),
                "Tolerance Matches": len(matched_with_tolerance_df),
                "Aggregated Matches": len(matched_aggregated) if 'matched_aggregated' in locals() else 0
            }
        }
