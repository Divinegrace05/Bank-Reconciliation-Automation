...
def save_all_reports(df):
    """Enhanced saving function with detailed error reporting and atomic writes"""
    try:
        # Debug filesystem first
        print(f"Attempting to save reports to: {REPORTS_FILE}")  # Debug logging
        ...
        print(f"Writing to temporary file: {temp_path}")  # Debug logging
        df.to_csv(temp_path, index=False)
        
        # Verify the temp file was created
        if not os.path.exists(temp_path):
            error_msg = f"Temp file not created at {temp_path}"
        ...
        if not os.path.exists(REPORTS_FILE):
            error_msg = f"Final file not found at {REPORTS_FILE}"
            print(error_msg)  # Debug logging
            return False, error_msg
        
        print(f"Successfully saved reports to: {REPORTS_FILE}")  # Debug logging
        return True, f"Successfully saved to {REPORTS_FILE}"
            ...
            if recon_func:
                ...
                if summary: # A simple check to ensure summary was populated
                    ...
                    if st.button("Add Current Summary to Reports"):
                        # Get or create the reports DataFrame
                        if 'all_reconciliation_reports' not in st.session_state:
                            st.session_state.all_reconciliation_reports = load_all_reports()
                        
                        # Generate the new report
                        new_report_data = generate_excel_summary_row(
                            summary, 
                            st.session_state.selected_bank, 
                            selected_country, 
                            recon_month_year
                        )
                        new_report_data['Comments'] = user_comments
                        
                        # Create a new DataFrame for the report
                        new_report_df = pd.DataFrame([new_report_data])
                        
                        # Combine with existing reports
                        updated_reports_df = pd.concat(
                            [st.session_state.all_reconciliation_reports, new_report_df],
                            ignore_index=True
                        )
                        
                        # Save the reports
                        success, message = save_all_reports(updated_reports_df)
                        
                        if success:
                            # Update session state only after successful save
                            st.session_state.all_reconciliation_reports = updated_reports_df
                            st.success("✅ Report saved successfully!")
                            st.balloons()
                            
                            # Debug output
                            st.write("Debug Info:")
                            st.write(f"Reports path: {REPORTS_FILE}")
                            st.write(f"File exists after save: {os.path.exists(REPORTS_FILE)}")
                            
                            if os.path.exists(REPORTS_FILE):
                                try:
                                    test_df = pd.read_csv(REPORTS_FILE)
                                    st.write(f"Loaded {len(test_df)} records from file")
                                except Exception as e:
                                    st.error(f"Error reading back file: {str(e)}")
                        else:
...
def reports_page():
    ...
    with st.expander("Debug Information", expanded=True):
        ...
        if os.path.exists(REPORTS_FILE):
            st.write(f"**File Size:** `{os.path.getsize(REPORTS_FILE)} bytes`")
            try:
                test_df = pd.read_csv(REPORTS_FILE)
                st.write(f"**File Contains:** {len(test_df)} records")
                st.write(test_df)
            except Exception as e:
    ...
    try:
        ...
        if not reports_df.empty:
            ...
            for _, report in reports_df.iterrows():
                with st.expander(f"{report['Provider name']} - {report['Month & Year']}"):
                    col1, col2 = st.columns(2)
                    with col1:
                    ...
                    st.write(f"**Variance:** {report['Variance']:,.2f}")
            ...
            st.info("No reports have been saved yet. Please perform a reconciliation and add the summary to reports.")
